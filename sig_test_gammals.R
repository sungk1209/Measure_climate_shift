...
baseline_year <- c(1811:1840)
compar_past_year <- c(1901:1930)
compar_present_year <- c(1991:2020)
compar_future_year <- c(2081:2100)
...
### Extract many draws from a baseline and comparison year and jdate
  ...
  baseline_df <- model_df %>% filter(year(date) %in% baseline_year & month(date) == month_interest)
  compar_past <- model_df %>% filter(year(date) %in% compar_past_year & month(date) == month_interest)
  compar_present <- model_df %>% filter(year(date) %in% compar_present_year & month(date) == month_interest)
  compar_future <- model_df %>% filter(year(date) %in% compar_future_year & month(date) == month_interest)
  ...
  ggplot() %>%
    ...
    ### T test
# 
#   t.past <- t.test(baseline_df$spi_curr,compar_past$spi_curr)$p.value
#   t.present <- t.test(baseline_df$spi_curr,compar_present$spi_curr)$p.value
#   t.future <- t.test(baseline_df$spi_curr,compar_future$spi_curr)$p.value
  
  wetter_past <-  mean(compar_past$spi_curr <= baseline_df$spi_curr)
  wetter_present <- mean(compar_present$spi_curr <= baseline_df$spi_curr)
  wetter_future <- mean(compar_future$spi_curr <= baseline_df$spi_curr)

  temp_sig_t<- data.frame(huc  = nums[huc_num],
                          month = month_interest,
                          past  = wetter_past,
                          present = wetter_present,
                          future = wetter_future,
                          t.past  = t.past,
                          t.present = t.present,
                          t.future = t.future)
...
saveRDS(sig_ttest, file = paste0(output_path,"significant_test_585.rds"))
